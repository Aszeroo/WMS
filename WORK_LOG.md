...
## 2026-09-08 — Final Test Coverage and CI/CD Pipeline
- Ran comprehensive coverage reports for backend and frontend, achieving >90% coverage across both modules.
- Implemented GitHub Actions workflow `ci.yml` that triggers on every push to `main`. The workflow runs lint, unit & integration tests, builds, and security audits for both backend and frontend.
- Added deployment step using `vercel-action` to publish the production build to Vercel, with environment variables pulled from repository secrets.
- Configured post‑commit hook to run `npm run typecheck` and lint checks to enforce quality before pushes.
- Updated `README.md` and `REPORT.md` to reflect the new CI/CD pipeline and test coverage strategy.

## 2026-09-08 — End of System Improvements
- All pending tasks from the improvement list are now marked complete.
- The system now runs a full automated CI pipeline, with high test coverage and clear audit trails.
- Future work may include further security hardening, performance tuning, and feature refinements.

## 2026-09-07 (fix development environment schema loading)
- Fixed development environment issue where `tsx watch src/server.ts` was causing Prisma to load the wrong schema file (`prisma/sqlite-test/schema.prisma` instead of `prisma/schema.prisma`).
- Modified the `dev` script in `backend/package.json` to set the `PRISMA_SCHEMA` environment variable:
  ```json
  "dev": "PRISMA_SCHEMA=./prisma/schema.prisma tsx watch src/server.ts"
  ```
- Verified that the backend now starts correctly and connects to the Supabase PostgreSQL database.
- Tested data storage by creating equipment types and instances via the API, confirming data is persisted in Supabase.
- The system is now fully functional in development mode, allowing local development and testing against the same PostgreSQL database used in production.
...
# ข้อมูลอ้างอิงและคู่มือการพัฒนา (References & Guides)
